...
def calc_dieviation(new_items, ext_avg, total_dur, exp_avg):
    new_avg = (sum([item[7]*item[1] for item in new_items])+(ext_avg*total_dur)) / (total_dur + sum([item[7] for item in new_items])) 
...
def remix(exp_time, diff, school="uvic", department="CSC", course_code="110"):
    ...
    while time_diff > 0.2 or time <= exp_time:
        ...
        print("TIME_DIFF: %s, TIME: %s, EXP_TIME: %s" % (time_diff, time, exp_time))
        print(candidates[0][0][0][0][:10], candidates[0][0][0][7])
        new_time = time + sum([q[7] for q in candidates[0][0]])
        while candidates and new_time > exp_time*1.2:
            candidates.pop(0) 
